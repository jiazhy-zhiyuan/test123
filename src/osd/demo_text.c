        ...
        char* osdMem, UInt32 lineOffset,UInt32* padingColor,UInt32 pixPerLine)
{
    ...
    switch(font.format){
        ...
        case SYSTEM_DF_YUV420SP_UV:
            ...
            {
                temp = osdMem + factor * i * lineOffset * scaleX;
                ...
                for(j = 0; j < textLen;j++)
                {
                    ...
                    int k = 0;
                    for(k = 0; k < font.widthList[index[j]] * scaleX; ++k){
                        if(src[k] < 0x20){
                            *temp++ = 0;
                            *temp++ = 0;
                        }else {
                    ...
                    //temp += factor * font.widthList[index[j]]*scaleX;
                }
            ...
            {
                temp = osdMem + factor * i* lineOffset*scaleX + factor * uvOffset;
                //process each character
                for(j = 0; j < textLen;j++)
                {
...
void Demo_text_draw(Bitmap_Info font, AlgLink_OsdWindowPrm* winPrm, char* text, UInt16 textLen, char* osdMem,UInt32* padingColor)
{
    ...
    for(i = 0;i < textLen; i++)
    {
        winPrm->width += 2 * font.widthList[index[i]];
    }
    ...
    {
        winPrm->addr[0][1] = winPrm->addr[0][0] + winPrm->lineOffset * 2 * font.height;
    }
