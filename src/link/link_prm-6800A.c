...
SystemVideo_Ivahd2ChMap_Tbl systemVid_encDecIvaChMapTbl =
{
	.isPopulated = 1,
	.ivaMap[0] =
	{
...
#define MULTICH_NUM_SWMS_0_BUFFERS			(8)
#define MULTICH_NUM_SWMS_1_BUFFERS			(8)
#define MULTICH_NUM_SWMS_2_BUFFERS			(8)
#define MULTICH_NUM_SWMS_3_BUFFERS			(8)
// 最大可以5个拼接器: 3, 4, 5, 6, 7
Void multich_set_swms_prm(UInt32 swMsId)
{
	...
	switch(swMsId) {
		case 0:
		{
			...
			g_swms[swMsId].swMsInstId[0]		= SYSTEM_SW_MS_SC_INST_SC5;	// SYSTEM_SW_MS_SC_INST_SC5
			g_swms[swMsId].maxInputQueLen		= SYSTEM_SW_MS_DEFAULT_INPUT_QUE_LEN + 6;
			g_swms[swMsId].numOutBuf		= NUM_BUFS_PER_CH_SWMS;
		...
		case 1:
		{
			...
			g_swms[swMsId].numSwMsInst		= 1;
			g_swms[swMsId].swMsInstId[0]		= SYSTEM_SW_MS_SC_INST_DEI_SC_NO_DEI;
			g_swms[swMsId].maxInputQueLen		= SYSTEM_SW_MS_DEFAULT_INPUT_QUE_LEN + 6;
			g_swms[swMsId].numOutBuf		= NUM_BUFS_PER_CH_SWMS;
		...
		{
			g_swms[swMsId].numSwMsInst		= 1;
			g_swms[swMsId].swMsInstId[0]		= SYSTEM_SW_MS_SC_INST_DEIHQ_SC_NO_DEI;
		...
		case 3:
		{
			...
			g_swms[swMsId].numOutBuf		= MULTICH_NUM_SWMS_3_BUFFERS;
...
Void multich_hddvr_set_swms_prm2(SwMsLink_CreateParams *swMsPrm,
                                    UInt32 swMsIdx)
{
    ...
    swMsPrm->maxOutRes                   = VSYS_STD_1080P_30;
...
Void multich_set_sd_display_res(VSYS_VIDEO_STANDARD_E resolution)
{
...
Void multich_set_display_prms(DisplayLink_CreateParams *displayPrm, int displayId)
{
	...
	{
		multich_set_sd_display_res(VSYS_STD_PAL);
	}

	displayPrm->displayRes = gVdisModuleContext.vdisConfig.deviceParams[VDIS_DEV_SD].resolution;
}

#define     ENC_LINK_SECONDARY_STREAM_POOL_ID    (0)	// 次码流
#define     ENC_LINK_PRIMARY_STREAM_POOL_ID      (1)	// 主码流
...
Void multich_hddvr_set_declink_prms(DecLink_CreateParams *decPrm)
{
	...
	for (i=0; i<gVdecModuleContext.vdecConfig.numChn; i++)
	{
        ...
        decPrm->chCreateParams[i].numBufPerCh            = 10;
		decPrm->chCreateParams[i].tilerEnable            = FALSE; //TILER_ENABLE_DECODE;
...
Void multich_hddvr_set_swms_prm(SwMsLink_CreateParams *swMsPrm, UInt32 swMsIdx)
{
	UInt32 devId, maxOutRes;
